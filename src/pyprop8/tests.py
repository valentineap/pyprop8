...
def test_spatial_derivatives(
    ...
    if source.nsources > 1:
        raise ValueError("test_derivatives requires a single force/mechanism")
    ...
    deriv_comp = lambda sl: tuple(
        nDimSta * [slice(None)]
        + nDimDerivs * [sl]
        + nDimChannels * [slice(None)]
        + nDimTime * [slice(None)]
    )
...
if __name__ == "__main__":
    ...
    source = pp.PointSource(
        ...
        0,
        20,
        rtf2xyz(make_moment_tensor(340, 90, 0, 2.4e8, 0, 0)),
        np.zeros([3, 1]),
        0,
    )
    ...
    if max(errs) > 1e-4:
        print("")
        print(
            "*** Warning: Mismatch between analytic and finite-difference derivatives? ***"
        )
        print("")
    else:
