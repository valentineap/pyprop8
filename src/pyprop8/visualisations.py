...
def generate_surface_movie(structure, source, stf=None):
    ...
    stations = RegularlyDistributedReceivers(
        1, 100, 100, 0, 360, 360, depth=depth, degrees=True
    )
    ...
    tt, seis = compute_seismograms(
        structure, source, stations, nt, dt, alpha, source_time_function=stf, pad_frac=2
    )
    amax = abs(seis[:, :, 2, :]).max()
        ...
        ls.shade(
            zero_centre(
                griddata(
                    ...
                    seis[:, :, 2, 0].flatten(),
                    np.array([xxg.flatten(), yyg.flatten()]).T,
                    method="linear",
                ).reshape(ngrid, ngrid)
            ),
            cmap=plt.cm.RdBu,
    ...
    a = anim.FuncAnimation(
        fig, make_frame, tqdm.tqdm(np.arange(1, nt)), blit=True, interval=40
    )
    # rprog.close()
    a.save("test.mp4")
...
def generate_surface_movie2(structure, source, stf=None):
    ...
    print("Generating seismograms...")
    stations = RegularlyDistributedReceivers(
        1, 100, 101, 0, 360, 360, depth=depth, degrees=True
    )
    # stations = ListOfReceivers()
    # ngrid = 25
    # xx,yy = np.meshgrid(np.linspace(-100,100,ngrid),np.linspace(-100,100,ngrid))
    ...
    a = anim.FuncAnimation(
        fig, make_frame, tqdm.tqdm(np.arange(1, nt)), blit=True, interval=50
    )
    # rprog.close()
    a.save("test.mp4")
    # plt.close(fig)
