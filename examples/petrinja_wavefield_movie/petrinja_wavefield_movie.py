...
model = pp.LayeredStructureModel([[ 1.0,  2.5, 1.1, 2.1],
                                  [ 1.0,  4.6, 2.6, 2.5],
                                  [ 9.9,  6.1, 3.6, 2.7],
                                  [ 9.9,  6.3, 3.7, 2.6],
                                  [10.2,  6.6, 3.6, 2.9],
...
moment_tensor_rtf = 1e6*np.array([[ 0.173,-0.022,-1.270],
                                  [-0.022,-4.630,-0.376],
                                  [-1.270,-0.376, 4.450]])
...
stf = lambda w: stf_trapezoidal(w, 3.8, 7.6)
...
scale_horiz = 0.35 #km/mm i.e. amplification of 35,000 x
scale_vert = 0.35 #km/mm
...
ax_visible = False
ncmax = 5
ax_range = (-100,100,-100,100,-ncmax*cmax,cmax) #Defines volume of view
focalpoint=(0,0,-2*cmax) # Where we are looking
...
do_wavefield=False # Useful to be able to turn this off and skip straight to second loop
if do_wavefield:
    for i in tqdm.tqdm(range(nt)):
        ...
        mlab.imshow(accstep.T,colormap='bone',extent=[-box_size,box_size,-box_size,box_size,-ncmax*cmax,-ncmax*cmax],vmin=0,vmax=maxacc)

        # Label the lower image
        mlab.text3d(-box_size-ticksize,-4*ticksize,-ncmax*cmax,"%i x %ikm"%(2*box_size,2*box_size),scale=5,orient_to_camera=False,orientation=(0,0,90))
        mlab.text3d(-box_size+5*ticksize,box_size+ticksize,-ncmax*cmax,"Ground acceleration",scale=5,orient_to_camera=False,orientation=(0,0,0))
        ...
        mlab.text(0.025,0.93,"29 Dec 2020 - Petrinja, Croatia - Mw6.4 at 12.0km depth",width=0.95)
        mlab.text(0.025,0.425,"Surface\ndisplacement\n(x35,000)",width=0.225)
...
for i in tqdm.tqdm(range(nt,nt+360)):
    ...
    img = mlab.imshow((insar.dot(los_vector).T+14)%28-14,colormap='jet',
                    extent=[-box_size,box_size,-box_size,box_size,-ncmax*cmax,-ncmax*cmax],
                    vmin=-14,vmax=14)
    ...
    # Label the lower image
    mlab.text3d(-box_size-ticksize,-4*ticksize,-ncmax*cmax,"%i x %ikm"%(2*box_size,2*box_size),scale=5,orient_to_camera=False,orientation=(0,0,90))
    mlab.text3d(-box_size-0.5*ticksize,box_size+ticksize,-ncmax*cmax,"Wrapped line-of-sight displacement",scale=5,orient_to_camera=False,orientation=(0,0,0))
